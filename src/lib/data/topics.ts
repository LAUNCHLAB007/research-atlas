...
export async function countEntriesInDomain(domainId: number): Promise<number> {
  const supabase = await createClient();
  const { count, error } = await supabase
    .from("topic_links")
    .select("id, topic:topics!inner(domain_id)", { count: "exact", head: true })
    .eq("topic.domain_id", domainId)
